...
int main() {
	...
	Method<n> * f[] = {
		new AvgHybrid<n,m>(&problem, "Avg. Hybrid (α=.75,ζ_{k}=1/k)", 1., .75, H, forget_lin<0,1>),
		new AvgHybrid<n,m>(&problem, "Avg. Hybrid (α=.75,ζ_{k}=2/(k+1))", 1., .75, H, forget_lin<1,1>),
		new AvgHybrid<n,m>(&problem, "Avg. Hybrid (α=.75,ζ_{k}=9/(k+8))", 1., .75, H, forget_lin<8,1>),
	};
