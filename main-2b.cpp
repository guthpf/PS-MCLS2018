...
int main() {
	...
	Method<n> * f[] = {
		new Hybrid<n,m>(&problem, "IP-SGD Hybrid (ζ_{k}=(2/(k+1))^{.4})", 1., 0., H, forget_pow<1,1,4,10>),
		new Hybrid<n,m>(&problem, "IP-SGD Hybrid (ζ_{k}=(2/(k+1))^{.8})", 1., 0., H, forget_pow<1,1,8,10>),
		new Hybrid<n,m>(&problem, "IP-SGD Hybrid (ζ_{k}=2/(k+1))", 1., 0., H, forget_pow<1,1,1,1>),
	};
